...
def submit_post(
    ...
    for area in _expertise_areas:
        ...
        if truth_rating and area.get("truth_rating").numeric_value < 0:
            try:
                fame_obj = Fame.objects.get(user=user, expertise_area=expertise_area)
                current_fame_level = fame_obj.fame_level
                try:
                    # lower the fame to the next possible level
                    new_fame_level = current_fame_level.get_next_lower_fame_level()
                    fame_obj.fame_level = new_fame_level
                    fame_obj.save()
                except ValueError:
                    # cnnot lower any further, ban the user
                    user.is_active = False
                    user.save()
                    redirect_to_logout = True
                    # Unpublish all posts by this user
                    Posts.objects.filter(author=user).update(published=False)
            except Fame.DoesNotExist:
                # add entry with Confuser fam level
        ...
        # fetch fame in expertise area and super pro
        fame = Fame.objects.filter(user=user, expertise_area=expertise_area).first()
        super_pro = FameLevels.objects.filter(name="Super Pro").first()

        # only remove community if both exist and fame is below Super Pro
        if fame and super_pro and fame.fame_level.numeric_value < super_pro.numeric_value:
